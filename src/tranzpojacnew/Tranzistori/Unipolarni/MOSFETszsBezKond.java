...
public class MOSFETszsBezKond {
    ...
    public MOSFETszsBezKond(double udd, double ugso, double k, double mi, double r1, double r2, double rs, double rD, double rt, double rg, double uul) {
        this.udd = udd;
        this.ugso = ugso;
        this.k = k;
        this.mi = mi;
        ...
        av = -(mi*((rD*rt)/(rD+rt)))/(((1+mi)*rs)+rd+((rD*rt)/(rD+rt)));
        rul = (r1*r2)/(r1+r2);
        ai = -av * rul/rt;
        miRsRd = ((1+mi)*rs)+rd;
        rizl = (rD*miRsRd)/(rD+miRsRd);
        ...
        RdRt = (-1/((rD*rt)/(rD+rt)+rs)) * 1000; //mnozenje zbog mA
        idqudsq = (idq + (udsq/((rD*rt)/(rD+rt)+rs))) * 1000; //mnozenje zbog mA
